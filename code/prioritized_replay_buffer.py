...
class RolloutStorage(object):
    ...
    def sample(self, mini_batch_size=None, probability_array=None):
        weight_array = np.power((probability_array / probability_array.min()), -self.beta)
        probability_array = np.power(probability_array, self.alpha)
        indices = []
        if probability_array is None:
            return np.array(indices)
        total = np.sum(probability_array)
        random_values = np.random.random(mini_batch_size) * total
        for random_value in random_values:
            current_probability = 0
            for i in range(probability_array.size):
                current_probability += probability_array[i]
        ...
        return obs_batch, obs_next_batch, actions_batch, rewards_batch, masks_batch, weight_array
